...
const AccountantDashboard = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [workersRes, projectsRes, fundingRes, procRes, paymentsRes, reportRes, statsRes] = await Promise.all([
        api.get('/api/workers'),
        ...
        api.get('/api/reports/accountant/stats'),
      ]);
      ...
      // Project spending
      const projectSpending = Object.entries(statsData.projectSpending || {}).map(([key, value]) => {
        const project = projectsRes.data.find(p => p._id === key);
        return { name: project?.name || key, amount: value };
      });

      // Payment trends
      const paymentTrends = Object.entries(statsData.paymentTrends || {}).map(([key, value]) => ({
        date: key,
        amount: value,
      ...
      // Approval ratio
      const approvalRatio = [
        { name: 'Pending', value: statsData.pendingFunding || 0 },
        { name: 'Approved', value: fundingRes.data.filter(f => f.status === 'approved').length },
        { name: 'Rejected', value: fundingRes.data.filter(f => f.status === 'rejected').length },
      ].filter(item => item.value > 0);

      // Top workers
      const workerEarnings = Object.entries(statsData.workerEarnings || {})
        .map(([key, value]) => {
          const worker = workersRes.data.find(w => w._id === key);
          return { name: worker?.name || key, amount: value };
        })
        .sort((a, b) => b.amount - a.amount)
        .slice(0, 5);

      setChartData({ projectSpending, paymentTrends, approvalRatio, topWorkers: workerEarnings });
      ...
      // Notification count
      const pendingFunding = fundingRes.data.filter(f => f.status === 'pending').length;
      const pendingBOQs = []; // we'd need to fetch BOQs separately
    ...
    <Box>
      ...
      {loading ? <CircularProgress /> : (
        <>
              ...
              {chartData.projectSpending.length > 0 && (
                <Grid item xs={12} md={6}>
                  <Paper sx={{ p: 2 }}>
                    ...
                    <BarChart width={400} height={200} data={chartData.projectSpending.slice(0, 5)}>
          ...
          {/* Bar Chart – Funding Requests vs Approved */}
          <Paper sx={{ p: 2 }}>
