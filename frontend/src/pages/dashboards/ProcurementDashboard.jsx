...
const ProcurementDashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const funded = ordersRes.data.filter(o => o.status === 'funded').length;
      const purchased = ordersRes.data.filter(o => o.status === 'purchased').length;
      ...
      {loading ? (
        ...
        <>
          <Grid container spacing={3} sx={{ mb: 3 }}>
            ...
            <Grid item xs={12} sm={3}>
              <Card><CardContent>
                <Typography variant="body2" color="textSecondary">Pending (blank amounts)</Typography>
            ...
            <Grid item xs={12} sm={3}>
              <Card><CardContent>
                ...
                <Typography variant="h4" color="success.main">{stats.funded}</Typography>
              </CardContent></Card>
            </Grid>
            <Grid item xs={12} sm={3}>
              <Card><CardContent>
              ...
              <TableBody>
                {orders.map(o => (
                  <TableRow key={o._id}>
                    ...
                    <TableCell>
                      <Chip label={o.status} color={o.status === 'funded' ? 'success' : o.status === 'purchased' ? 'info' : 'warning'} />
                    </TableCell>
                    ...
                    <TableCell>
                      ...
                      {o.status === 'pending' && (
                        <Button size="small" color="primary" onClick={() => handleFund(o._id)}>Fund</Button>
                      )}
                    </TableCell>
                  </TableRow>
                ))}
          ...
          <Dialog open={openModal} onClose={() => setOpenModal(false)} maxWidth="md" fullWidth>
            ...
            <DialogContent>
              ...
              <Typography variant="h6" sx={{ mt: 2 }}>
                Grand Total: {getGrandTotal(form.items).toFixed(2)}
              </Typography>
            </DialogContent>
            <DialogActions>
