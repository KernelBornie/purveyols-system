...
import {
  ...
  Chip, Paper, CircularProgress, Divider, IconButton
} from '@mui/material';
...
const DirectorDashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const pendingBOQs = boqRes.data.filter(b => b.status === 'draft' || b.status === 'submitted').length;

      setStats({
        totalProjects: projectsRes.data.length,
        activeProjects,
        totalFundingRequests: fundingRes.data.length,
        pendingFunding,
        totalBOQs: boqRes.data.length,
        pendingBOQs,
        totalWorkers: workersRes.data.length,
        totalSubcontracts: subRes.data.length,
      });
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const fundingChartData = fundingRequests.map(f => ({
    name: f.project?.name || 'N/A',
    amount: f.amount,
    status: f.status,
  }));
          ...
          <Grid container spacing={3} sx={{ mb: 3 }}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Card><CardContent>
                ...
                <Typography variant="caption" color="warning.main">
                  {stats.pendingBOQs} pending approval
                </Typography>
              </CardContent></Card>
            </Grid>
            <Grid item xs={12} sm={6} md={3}>
              <Card><CardContent>
                <Typography variant="body2" color="textSecondary">Workers / Subcontracts</Typography>
