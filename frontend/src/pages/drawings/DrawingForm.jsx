...
const DrawingForm = () => {
  ...
  const addRectangle = () => {
    const fc = getCanvas();
    if (!fc) return;
    fc.add(new fabric.Rect({ left: 100, top: 100, width: 120, height: 80, fill: 'rgba(0,123,255,0.1)', stroke: '#007bff', strokeWidth: 2 }));
    fc.renderAll();
  };
  const addCircle = () => {
    const fc = getCanvas();
    if (!fc) return;
    fc.add(new fabric.Circle({ left: 150, top: 150, radius: 50, fill: 'rgba(255,0,0,0.1)', stroke: '#dc3545', strokeWidth: 2 }));
    fc.renderAll();
  };
  const addLine = () => {
    const fc = getCanvas();
  ...
  const addText = () => {
    const fc = getCanvas();
    if (!fc) return;
    fc.add(new fabric.Textbox('Edit me', { left: 200, top: 200, width: 200, fontSize: 20, textBaseline: 'alphabetic' }));
    fc.renderAll();
  };
  ...
  const clearCanvas = () => {
    ...
    fc.clear();
    const gs = 10;
    for (let i = 0; i < 1200; i += gs) fc.add(new fabric.Line([i, 0, i, 800], { stroke: '#e8e8e8', selectable: false, evented: false, excludeFromExport: true }));
    for (let i = 0; i < 800; i += gs) fc.add(new fabric.Line([0, i, 1200, i], { stroke: '#e8e8e8', selectable: false, evented: false, excludeFromExport: true }));
    fc.renderAll();
  };
  const saveDrawing = () => {
    const fc = getCanvas();
  ...
  const addPanel = () => {
    ...
    for (let i = 0; i < 3; i++) fc.add(new fabric.Line([210, 320 + i * 20, 250, 320 + i * 20], { stroke: '#333', strokeWidth: 1 }));
    fc.renderAll();
  };
  const addCable = () => {
    const fc = getCanvas();
  ...
  // ================================================================
  //  EVALUATION (Boreholes, Test Pits)
  // ================================================================
  const addBorehole = () => {
    const fc = getCanvas();
    if (!fc) return;
  ...
  // ================================================================
  //  WATER TABLE DETERMINATION (Piezometers, Groundwater)
  // ================================================================
  const addPiezometer = () => {
    const fc = getCanvas();
    if (!fc) return;
  ...
  const addContour = () => {
    ...
    fc.add(new fabric.Polyline(pts.map(p => [p.x, p.y]), { stroke: '#8b0000', strokeWidth: 2, fill: null }));
    fc.renderAll();
  };
  ...
  const addCCTVCable = () => {
    const fc = getCanvas();
    if (!fc) return;
    fc.add(new fabric.Line([680, 200, 730, 200], { stroke: '#ff4500', strokeWidth: 2, strokeDashArray: [4, 4] }));
    fc.renderAll();
  };
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Paper sx={{ p: 1, mb: 2, overflowX: 'auto', whiteSpace: 'nowrap' }} elevation={2}>
        ...
        <ButtonGroup variant="outlined" size="small" sx={{ mr: 1 }}>
          <Tooltip title="Rect"><Button onClick={addRectangle}><RectIcon /></Button></Tooltip>
          ...
          <Tooltip title="Pen"><Button onClick={toggleFreehand} color={activeTool === 'pen' ? 'primary' : 'inherit'}><PenIcon /></Button></Tooltip>
        </ButtonGroup>
        ...
        {/* Evaluation & Water Table */}
        <CategoryLabel>Evaluation / WT</CategoryLabel>
        <ButtonGroup variant="outlined" size="small" sx={{ mr: 1 }}>
      ...
      <Box sx={{ border: '2px solid #ccc', borderRadius: 2, overflow: 'auto', width: '100%', minHeight: '800px', bgcolor: '#fafafa', position: 'relative' }}>
        {canvasLoading && <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }}><CircularProgress /></Box>}
        <canvas ref={canvasRef} width={1200} height={800} style={{ display: 'block', width: '100%', height: 'auto' }} />
      </Box>
      <Typography variant="caption" sx={{ mt: 1, display: 'block' }}>
        {canvasReady ? '✅ All tools loaded – start drawing' : '⏳ Initializing...'}
      </Typography>
    </Box>
  );
};
