      ...
      <form onSubmit={handleSubmit}>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={12} md={6}>
            <TextField label="M/S" fullWidth size="small" value={form.ms} onChange={e => setForm({ ...form, ms: e.target.value })} placeholder="Customer name..." />
          </Grid>
          <Grid item xs={12} md={6}>
            <TextField label="Date" type="date" fullWidth size="small" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} InputLabelProps={{ shrink: true }} />
          </Grid>
        </Grid>
            ...
            {form.items.map((item, idx) => (
              <TableRow key={idx}>
                <TableCell sx={{ border: '1px solid #000', p: 1 }}>
                  <TextField size="small" fullWidth value={item.quantity} onChange={e => handleItemChange(idx, 'quantity', e.target.value)} placeholder="e.g., 5" sx={{ '& .MuiInputBase-root': { border: 'none' } }} />
                </TableCell>
                <TableCell sx={{ border: '1px solid #000', p: 1 }}>
                  <TextField size="small" fullWidth value={item.description} onChange={e => handleItemChange(idx, 'description', e.target.value)} placeholder="Item description..." sx={{ '& .MuiInputBase-root': { border: 'none' } }} />
                </TableCell>
        ...
        <Grid container spacing={2} sx={{ mt: 3 }}>
          <Grid item xs={12} md={6}>
            <TextField label="Delivered By" fullWidth size="small" value={form.deliveredBy} onChange={e => setForm({ ...form, deliveredBy: e.target.value })} placeholder="Name of deliverer..." />
          </Grid>
          <Grid item xs={12} md={6}>
            <TextField label="Received By" fullWidth size="small" value={form.receivedBy} onChange={e => setForm({ ...form, receivedBy: e.target.value })} placeholder="Name of receiver..." />
          </Grid>
        </Grid>
