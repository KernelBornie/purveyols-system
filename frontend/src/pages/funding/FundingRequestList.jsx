...
const FundingRequestList = () => {
  ...
  const handleApprove = async (id) => {
    try {
      ...
      setRequests(prev =>
        prev.map(r => r._id === id ? (res.data.request || res.data) : r)
      );
    } catch {
  ...
  const handleReject = async (id) => {
    ...
    try {
      const res = await API.put(`/funding-requests/${id}/reject`, {
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <Link to="/funding-requests/new" className="btn btn-primary">
          + New Request
        </Link>
      </div>
                    ...
                    <td>
                      <div className="actions">
                        ...
                        {['engineer', 'director'].includes(user?.role) && (
                          <Link
                            to={`/funding-requests/${req._id}/edit`}
                        ...
                        {req.rejectionReason && (
                          <span
                            ...
                            style={{ cursor: 'help', color: '#e74c3c' }}
                          >
                            ⚠
                          </span>
                        )}
