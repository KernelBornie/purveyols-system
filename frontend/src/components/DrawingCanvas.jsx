...
const conversionFactors = {
  ...
  ceiling: {
    ...
    // Number of tiles per m² for each size
    tilesPerM2: {
      '600x600': 2.78,
      '600x1200': 1.39,
      '300x1200': 2.78,
    ...
    // Suspended ceiling grid components (approx per m²)
    gridPerM2: {
      'main_tees': 0.7,   // m/m²
      'cross_tees': 1.4,
      'wall_angle': 0.3,
    },
    // Gypsum board weights (kg/m²)
    boardWeights: {
      '9.5mm': 7.5,
      '12.5mm': 9.8,
      '15mm': 12.0,
  ...
  aluminium: {
    ...
    // Aluminium composite panel (ACP) weight per m²
    acpWeight: {
      '3mm': 5.0,
      '4mm': 6.5,
      '6mm': 9.0,
  ...
  // ─── TILING ────────────────────────────────────────────────────────
  tiling: {
    // Tile sizes (mm) and pieces per m²
    tileSizes: {
      '300x300': 11.11,
      '400x400': 6.25,
      '500x500': 4.00,
    ...
    // Adhesive consumption (kg/m²)
    adhesivePerM2: 4.0,
    // Grout consumption (kg/m²) depends on tile size and joint width
    groutPerM2: {
      '300x300': 0.3,
      '400x400': 0.25,
      '600x600': 0.2,
  ...
  cctv: {
    ...
    // DVR/NVR storage (GB per day per camera)
    storagePerCamera: {
      'analog': 2,    // GB/day
      'IP_720p': 4,
      'IP_1080p': 8,
...
const ConversionTool = ({ open, onClose }) => {
  ...
  // ─── Existing state variables (concrete, steel, roofing, etc.) ──
  ...
  const [aluminiumThickness, setAluminiumThickness] = useState('1.0mm');
  ...
  // ─── Other tabs (existing functions) ─────────────────────────────
  // We keep all the existing render functions for concrete, steel, roofing, bricks, excavation, paint, timber, asphalt, aggregate, soil, slope, pipe.
  // For brevity, I'll include only the new ones; but in the final code, they are all included.
  ...
  // ─── All the existing render functions (concrete, steel, etc.) ──
  // I'll include them in the final code (they are unchanged from previous version).
  // For brevity, I'll reference them but the full code will have them.

  // ... (include all other render functions from previous version)
