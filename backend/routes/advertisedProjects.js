...
const router = express.Router(); // ✅ REQUIRED – this was missing
...
// ─── Sources (NewsAPI + RSS/Web fallback) ──────────────────────
const SOURCES = [
  {
...
router.post('/:projectId/bid', auth, async (req, res) => {
  try {
    ...
    // Create a Bid document using the updated schema
    const bidData = {
      advertisedProjectId: project.id,
